...
/// Generate the backup-run.sh orchestrator script with DAS_SERIALS, source/target
/// mount vars, device vars, log functions, mount logic, btrbk run, and indexer walk.
pub fn render_backup_run(config: &Config) -> String {
    ...
    // Core functions (log, cleanup, etc.)
    out.push_str(
        "# ============================================================================\n",
    );
...
"#,
    ...
    for source in &config.sources {
        ...
        out.push_str(&format!(
            "mkdir -p \"$MOUNT_{var_name}\"\n\
             mountpoint -q \"$MOUNT_{var_name}\" || \
             mount -o subvolid=5 \"$DEV_{var_name}\" \"$MOUNT_{var_name}\"\n"
        ));
    }
    out.push('\n');
    ...
    for target in &config.targets {
        if target.role == TargetRole::Primary || target.role == TargetRole::Mirror {
            out.push_str(&format!(
                "\"$BTRDASD_BIN\" walk \"{}\" --db \"$DB_PATH\" || \
                 log \"WARN\" \"Indexing {} failed (non-fatal)\"\n",
...
impl GeneratedFiles {
    ...
    pub fn generate(config: &Config) -> Self {
        ...
        // backup-run.sh (needs +x at install time)
        files.push((
