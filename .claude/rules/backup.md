...
- **The two paths diverge on mirror-target scoping**: the bash path explicitly skips `role=mirror` targets (`update_boot_subvolumes()` checks `$mount_role == "mirror"` and logs `"Skipping mirror target (independent OS)"`) because `system-recovery-A-2tb`/`system-recovery-B-2tb` carry their own independent `das-recovery-bay1` Arch install under `@`/`@home` — archiving/recreating those would destroy a bootable OS that has nothing to do with this host's boot subvolumes. The Rust `archive_boot()` currently iterates **all** configured targets with no role filter, so it does not skip mirror targets. Tracked as `bd DAS-Backup-Manager-am1`.
- **Warning**: until `am1` is fixed, running `btrdasd backup run` manually will archive and replace `@`/`@home` on the mirror recovery targets too — avoid manual runs against configs with mirror targets configured until that bug is resolved.
...
- **`das-scrub.service` has the same clean profile** (no `Restart=`, no `OnFailure=`, no `OnUnitInactiveSec=` — verified in `/etc/systemd/system/das-scrub.service`) — the same safety analysis above applies: Sentinel only acts on a unit it observes in `failed` state, and it never touches `das-scrub.timer` itself (a timer unit doesn't go `failed` when its service exits nonzero; the *service* does, and only if `btrdasd scrub run` itself returns a nonzero exit — the engine's own locks make a "collision" outcome a deferral, not a failure). The manual-stop caveat is identical too: `systemctl stop das-scrub.service` alone will not survive a Sentinel retry; mask it first.
- **Scrub failure-retry loop (bd `DAS-Backup-Manager-18p`) — resolved Sentinel-side 2026-08-02.** A persistent scrub failure would have retry-looped indefinitely: Sentinel's restart rate limiter is 3 restarts per 600 s, and scrub failures land 14–18 h apart, so every failure got a fresh window and the limiter never engaged. The fix lives in the `cachyos-sentinel` project, deliberately NOT as a unit exclusion. Caveats passed back for future unit work here:
  ...
  - Do not rely on Sentinel's rate limiter to brake failure loops whose period exceeds ~10 minutes — it structurally cannot.
  - `das-scrub` keeps exit-1-on-failure semantics; health (Critical escalation) and the FAILURE email remain the alerting channels.
