...
the stream ships nothing and the run reports success. This is the single easiest
...
```bash
sudo btrfs subvolume snapshot -r /srv /.btrfs-ssd/.btrbk-snapshots/PROOF
sudo du -sb /.btrfs-ssd/.btrbk-snapshots/PROOF/VirtualMachines/   # 0, while the live path holds 34 GiB
...
```bash
for fs in /.btrfs-nvme /.btrfs-ssd /.btrfs-hdd /dasRaid0; do
  echo "== $fs"; sudo btrfs subvolume list "$fs" \
    | awk '{print $NF}' | grep -v '\.btrbk-snapshots\|\.snapshots\|\.archive\.' | sort
done
```

Deliberate omissions, so a future audit reads this as a diff rather than a
re-derivation: `@tmp`, `@var-tmp` (ephemeral), `@cache` and `coredumps` (listed in
...
which *was* backed up daily and successfully, and the VM was in none of it.
...
- **Both code paths archive before replacing, and BOTH now locate the replacement first** — scoped to non-mirror targets. The bash scheduled path is `scripts/backup-run.sh` `update_boot_subvolumes()` (`--full` runs only); the Rust manual path is `indexer/src/backup.rs` `archive_boot()` (D-Bus/GUI and manual `btrdasd` runs). Prior to 2026-08-01, `backup-run.sh` recreated `@`/`@home` with no archive step at all, and the archive step was taken from the Rust path (user decision 2026-08-01).
- **CORRECTION 2026-08-27 — the Rust path was NOT authoritative, and this file said it was.** Until 0.7.20.0 `archive_boot()` ran archive → **delete live `@`** → look up a replacement → recreate, so a failed lookup left the subvolume deleted behind a single `Warning`. `update_boot_subvolumes()` had the correct shape all along: look up first, `continue` if nothing is found, then create `@.new` → delete `@` → `mv`. **Bash was right; Rust was wrong; this rule asserted the opposite for 26 days.** Fixed in 0.7.20.0 (`bd DAS-Backup-Manager-5ig`) by giving the Rust path the same look-up-first ordering.
- **The ordering bug was only reachable because of a name mismatch**, and the pair is worth remembering as one failure. `find_latest_btrbk_snapshot` built its search prefix from a hardcoded `"@" => "root"` map, while the live `/etc/btrbk/btrbk.conf` carries `snapshot_name root-` — `resolve_snapshot_names` disambiguates the bare `@` because `@root` also resolves to `root`. On-disk `nvme/root-.<TS>` therefore never matched `nvme/root.`, so the lookup could never succeed for `@` and every Rust-path run on a primary target left the mount with no bootable `@`. **`@home` was unaffected throughout** — it maps to `home`, `btrbk.conf` carries a plain `home`, and it matched. An asymmetry like that is why "the code path is broken" and "the code path is broken *for this input*" are different findings.
- **Snapshot names are now read from `/etc/btrbk/btrbk.conf`, never re-derived.** `forget::live_subvol_snapshot_names()` maps each declared `subvolume` to the `snapshot_name` btrbk will actually write, extending the precedent `btrdasd forget` set in 0.7.16.0: the generated file is what btrbk consumes, so it cannot disagree with reality the way a second implementation of the naming rules could. Target subdirectories come from `Source.target_subdirs` for the same reason. **If `btrbk.conf` cannot be read, `archive_boot` declines the whole step** rather than guessing — a wrong name here used to cost the live subvolume.
